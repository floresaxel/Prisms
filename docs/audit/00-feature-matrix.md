...
| R7 | Server improves, never required offline | client-side status/aggregates in core | 🔎 | S2/S3 |
...
| R16 | Synced-row schema versioning separate from command versioning | `schema_version` col, floor check in dispatcher | 🔎 | S4/S6 |
...
| V5 | Additive-only synced schema; old clients ignore unknown columns | 🔎 | S6 |
| V6 | Automation template versioning; backstop checks content, raises drift review item | 🔎 | S5 |
| V7 | Incremental fact-keyed StatusIndex; no stored status; no full scan | 🔎 | S2 |
| V8 | Tier 0/1/2 streams before 100k load test | 🔎 | S6 |
| V9 | LWW default; explicit merges for sort_order + timer intervals | 🔎 | S2 |
