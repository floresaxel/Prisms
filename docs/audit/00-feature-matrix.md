...
**REMEDIATION COMPLETE (R1–R10, 2026-07-03; see [AUDIT_PLAN.md](AUDIT_PLAN.md) → Remediation).** **All 6 High findings closed** with commits: S3-F1 `05a3bc6` (R2) · S3-F2 `3a243c9` (R3) · **S7-F1 `5ca01c5` (R5)** · S7-F2 `a7e7a2b` (R6) · S8-F1 `d8eaecc` (R7) · S9-F1 + S9-F2 `ee02580` (R9). R10 `r10-topology-signoff`: V8 tier substance + `command_results` dropped + scoped publication, server hardening (S4-F4/F5/F6, S3-F7, S10-F5), the additive-schema gate (S2-F2) and two-user isolation test (S10-F3a), docs truth-up. R5 `5ca01c5`: offline automation spawning + invariant pre-flight + soft-delete closure + `depends_on` in the client write path. Rows below re-verdicted where a fix flipped them.
...
| R4 | Spawning, unblocking, clock-in/out, agenda edits, suggestion accept/reject offline | optimistic effect builders `packages/ui/src/powersync/effects.ts` + write-path guards `execute.ts` | ✅ **S7-F1 fixed (R5 `5ca01c5`)**: `execute()` runs core `runAutomations` on merged facts (byte-identical UUIDv5 spawns) + soft-delete closure (S7-F7) + invariant pre-flight (S7-F4) + `depends_on` (S7-F5); other verbs ✅ | S7 → R5 |
...
| R19 | External facts advisory only; never gate/diverge | weather badge display-only; engine ignores weather in convergent outcomes | ✅ **S3-F1 fixed (R2 `05a3bc6`)**: weather no longer causes `E_BLOCKED_TASK`; jobs side ✅ S5 (automations reject weather conditions, S5-F10). ✅ client pre-flight mirrors it (R5 `5ca01c5`) | S5/S3 → R2/R5 |
...
| V10 | External-fact state never gates rejection/convergence | ✅ **server (R2 `05a3bc6`)**: `isBlockedForAcceptance` excludes weather-reading rules so the clock-in gate no longer rejects on weather; weather-conditioned automations rejected at authoring (S3-F8). ✅ client pre-flight mirrors it (R5 `5ca01c5`: acceptance-safe clock-in guard, weather never blocks locally) | S3/S5 → R2/R5 |
