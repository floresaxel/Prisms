...
export async function runAggregatesRecompute(
  ...
  const [nodeRows, entryRows, habitRows, completionRows, blockRows, tagRows, placementRows, answerRows, settingsRow] = await Promise.all([
    ...
    db.select().from(tags).where(eq(tags.user_id, userId)),
    db.select().from(tag_placements).where(eq(tag_placements.user_id, userId)),
    db.select().from(tag_answers).where(eq(tag_answers.user_id, userId)),
